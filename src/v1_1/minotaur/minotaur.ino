...
// inputs and outputs
#define VCC_ARM_SW    3  // OUTPUT switch arm voltage
#define LEFT_ARM_PIN  4  // OUTPUT activate left arm
#define RIGHT_ARM_PIN 5  // OUTPUT activate right arm
#define LAMP_PIN      6  // OUTPUT activate lamp
...
#define BTN_FN        11 // INPUT  button function selected
...
void closeWhilePressed() {
  ...
  while (btnState[BTN_CLOSE] == BTN_ON) {
    lampOn = true;
    digitalWrite(btnState[BTN_FN] == BTN_ON ? RIGHT_ARM_PIN : LEFT_ARM_PIN, ARM_ON);
  }
...
void programCycle() {
  ...
  // open left arm
  while (btnState[BTN_SELECT] != BTN_ON); // wait for btn
